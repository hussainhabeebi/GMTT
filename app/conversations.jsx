...
// Resolve the best "lead-like" object for a conversation:
// 1. Match by leadId
// 2. Match by phone number (digits-only — robust to formatting differences)
// 3. Fall back to inline contact fields the workflow attached (contactName/contactPhone/contactEmail)
const phoneKey = (p) => String(p || '').replace(/\D/g, '');
const resolveLead = (c, leads) => {
  if (!c) return null;
  const byId = c.leadId && leads.find(l => l.id === c.leadId);
  if (byId) return byId;
  const cp = phoneKey(c.contactPhone || c.phone);
  if (cp) {
    const byPhone = leads.find(l => phoneKey(l.phone) === cp);
    if (byPhone) return byPhone;
  }
  // Synthesise a minimal lead from inline conversation fields
  if (c.contactName || c.contactPhone || c.contactEmail) {
    return {
      id: c.leadId || ('chat:' + c.id),
      name: c.contactName || c.contactPhone || 'Unknown contact',
      phone: c.contactPhone || '',
      email: c.contactEmail || '',
      state: '', degree: '', english: '', offDay: '', callTime: '',
      eligible: null, gulf: '-', stage: 'new', regYear: '',
      __synthetic: true,
    };
  }
  return null;
...
function Conversations({ leads, conversations: initial, onOpenLead, push }) {
  ...
  const filtered = convos
    ...
    .filter(c => {
      ...
      const lead = resolveLead(c, leads);
  ...
  const activeLead = active ? resolveLead(active, leads) : null;
  ...
  const send = async () => {
    ...
    if (res.live) push('Reply sent', { icon: 'check' });
    else push('Offline — reply queued locally', { icon: 'warn' });
  };
                ...
                <div key={c.id}
                  ...
                  <div className="row gap-10" style={{ alignItems: 'flex-start' }}>
                    <div style={{ position: 'relative' }}>
                      <Avatar name={lead?.name || c.contactName || 'Unknown'} />
                      <span style={{
                    ...
                    <div key={i} style={{
                      ...
                      {!mine && (
                        <div style={{ width: 28, flexShrink: 0 }}>
                          {showAvatar && <Avatar name={activeLead?.name || active?.contactName || 'Unknown'} size="sm" />}
                        </div>
                      )}
