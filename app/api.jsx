...
// Unwrap common n8n response shapes — many workflows return {data:[...]},
// {items:[...]}, {payload:[...]} or a bare object instead of a bare array.
const unwrap = (v) => {
  if (v == null) return v;
  ...
  if (typeof v === 'object') {
    ...
    if (Array.isArray(v.json))    return v.json;
...
async function call(endpointKey, opts, fallback) {
  try {
    const raw = await rawCall(endpointKey, opts);
    const data = unwrap(raw);
    _status = { online: true, lastSync: new Date().toISOString(), lastError: null, lastEndpoint: endpointKey };
    emit();
    try {
      const summary = Array.isArray(data) ? `${data.length} items` : (typeof data);
      console.log(`[GMTT api] ${endpointKey} ✓`, summary, data);
    } catch (e) {}
    return { data, live: true };
  } catch (err) {
    _status = { online: false, lastSync: _status.lastSync, lastError: String(err.message || err), lastEndpoint: endpointKey };
    emit();
    // Visible diagnostics in DevTools — CORS errors are otherwise opaque.
    try {
      console.warn(`[GMTT api] ${endpointKey} failed →`, err.message || err,
        '\n  URL:', resolveUrl(endpointKey),
        '\n  Likely fix: add CORS headers to the n8n Respond to Webhook node, or set Access-Control-Allow-Origin: *');
    } catch (e) {}
