...
function Today({ leads, conversations, status, onOpenLead, onJump }) {
  ...
  return (
    <div>
      <PageHeader
        ...
        right={[
          status?.online === true ? (
            <span key="syncing" className="pill pill--ok pill--dot">
              Live · synced {status.lastSync ? new Date(status.lastSync).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'now'}
            </span>
          ) : status?.online === false ? (
            <span key="syncing" className="pill pill--warn pill--dot tt" data-tt={status.lastError || ''}
                  style={{ cursor: 'pointer' }} onClick={() => onJump('settings')}>
              n8n offline · showing sample data
            </span>
          ) : (
            <span key="syncing" className="pill pill--neutral pill--dot tt" data-tt="Wire n8n in Settings → Integrations"
                  style={{ cursor: 'pointer' }} onClick={() => onJump('settings')}>
              Sample data
            </span>
          ),
