...
function EmailComposer({ leads, push }) {
  ...
  const send = async () => {
    ...
    try {
      await apiPost('/guiders-send-email', {
        to_email: lead.email,
        subject: draftSubj,
        body: draftBody,
      ...
      push(`Sent (check SMTP logs)`, { icon: 'check' });
    }
  };
          ...
          <div className="spread">
            ...
            <div className="row gap-8">
              ...
              <button className="btn btn--primary btn--sm"
                onClick={async () => {
                  setBulkOpen(false);
                  push(`Sending ${eligiblePending.length} emails…`, { icon: 'send' });
                  let sent = 0;
                  for (const l of eligiblePending) {
                    const rendered2 = renderTemplate(EMAIL_TEMPLATES.find(t=>t.id===tmplId), l);
                    try {
                      await apiPost('/guiders-send-email', {
                        to_email: l.email, subject: rendered2.subject,
                        body: rendered2.body, lead_phone: l.phone, template_id: tmplId,
                      });
                      sent++;
                    } catch(e) {}
                    if (bulkDelay > 0) await new Promise(r => setTimeout(r, bulkDelay * 1000));
                  }
                  push(`✓ ${sent} emails sent`, { icon: 'check' });
                }}>
                Start sending
              </button>
            </div>
