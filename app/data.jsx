...
// ── Live data loader — fetches from Google Sheet via n8n ──────────────────
window.LIVE_LEADS = null; // populated on load
window.loadLiveLeads = async () => {
  try {
    ...
    return rows.map((r, i) => ({
      id: r['Phone Number'] ? 'L' + r['Phone Number'].replace(/\D/g,'').slice(-6) : `L${9000+i}`,
      ...
      eligible:    r.Eligible === 'Yes' ? true : r.Eligible === 'No' ? false : null,
      gulf:        r.Is_Gulf === 'Yes' ? (r.State || 'Gulf') : '-',
      stage:       r.Stage || 'new',
      regYear:     null,
      ...
      notes:       [],
      callLog:     [],
      emailHistory: r.Invite_Sent === 'Yes' ? [{ ts: 'Previously', template: 'Call Invite', status: 'sent' }] : [],
      lastContact: 'recently',
    }));
  } catch(e) {
    console.warn('Live leads fetch failed, using sample data', e);
    return null;
  }
};

// Expose sample data as fallback
window.SAMPLE_LEADS = LEADS;
