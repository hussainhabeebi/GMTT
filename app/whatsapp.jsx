...
function WhatsAppFollowup({ leads, push }) {
  ...
  const send = async () => {
    if (bulkMode) {
      push(`Queuing ${selectedIds.size} WhatsApp messages…`, { icon: 'whatsapp' });
      const res = await api.triggerAutomation('bulk_whatsapp', null, {
        leadIds: [...selectedIds],
        type: typeId,
        body: editedText,
      });
      if (res.live) push(`WhatsApp queued · ${selectedIds.size} leads`, { icon: 'check' });
      else push('n8n offline — WhatsApp not queued', { icon: 'warn' });
    } else {
      push(`Sending to ${firstSelected.name}…`, { icon: 'whatsapp' });
      const res = await api.sendWhatsApp({
        to: firstSelected.phone,
        leadId: firstSelected.id,
        body: editedText,
        type: typeId,
      });
      if (res.live) push(`Sent to ${firstSelected.name}`, { icon: 'check' });
      else push('n8n offline — message not sent', { icon: 'warn' });
    }
  };
