...
function WhatsAppFollowup({ leads, push }) {
  ...
  const send = async () => {
    if (bulkMode) {
      push(`Queuing bulk WhatsApp for ${selectedIds.size} leads…`, { icon: 'whatsapp' });
      try {
        await window.gmttPost(window.GMTT_API.AUTOMATION, {
          name: 'bulk_whatsapp',
          type_id: type.id,
          lead_ids: Array.from(selectedIds).map(id => {
            const l = leads.find(x => x.id === id);
            return l?.phone || null;
          }).filter(Boolean),
        });
        push(`✓ Bulk WhatsApp queued — n8n will send to ${selectedIds.size} leads`, { icon: 'check' });
      } catch(e) {
