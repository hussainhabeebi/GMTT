...
function App() {
  ...
  const loadAll = React.useCallback(async (silent = false) => {
    ...
    setError(null);
    try {
      const [liveLeads, liveConvs] = await Promise.all([
        window.loadLiveLeads(),
        window.loadLiveConversations('open'),
      ]);
      setLeads(liveLeads || []);
      ...
      setError('Could not connect to data source. Check n8n webhook is active.');
    }
    setLoading(false);
  }, []);
